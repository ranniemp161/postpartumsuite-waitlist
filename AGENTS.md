...
PostpartumSuite Waitlist - a Next.js waitlist site for PostpartumSuite.

> TODO: replace this with the real problem statement and scope. Fill in
> `blueprint/project-plan.md` and `blueprint/build-plan.md`, then run `/overview`.
...
Next.js 16 (App Router) with npm, verified against `package.json`.

- Dev server: `npm run dev` (http://localhost:3000)
- Build: `npm run build`
- Production server: `npm run start` (needs a build first)
- Lint: `npm run lint` (ESLint 9 flat config in `eslint.config.mjs`; Next 16 removed `next lint`, so the script calls `eslint` directly)

There is no typecheck script. `npm run build` type-checks as part of the build;
add a `typecheck` script (`tsc --noEmit`) if you want it as a separate gate.

**No test command is configured, so the test gate is off.** Logic-bearing steps
verify with the build, the running app, and screenshots instead. Run `/tests` or
`$tests` to add a stack-native unit runner and turn the gate on; that is what
adds a real `test` command here.

No `Verify` command and no GitHub workflow exist yet. Run `/ci` or `$ci` when you
want one Verify command plus matching automatic checks.
